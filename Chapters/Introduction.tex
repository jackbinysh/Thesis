...
When vorticity is not concentrated along a singular line, but in a thin tube, it may have additional internal structure --- one imagines a knotted ribbon (Figure \ref{fig:RibbonMontage}(b)), or rubber bicycle tyre (Figure \ref{fig:RibbonMontage}(a)), as opposed to a curve. Flux lines may wind around the centre-line of the vortex tube as in Figure \ref{fig:RibbonMontage}(a), endowing the tube with a second linking number, the Self-Linking number, which measures the linking of any flux tube with the curve centre-line, or equivalently the number of rotations this tube makes as we traverse the centre-line once. 
\begin{figure}[htbp]
\centering
...
\end{figure}
Incorporating this sturcture into the Helicity count, we find \cite{MoffatRicca}
\begin{equation}
...
The hydrodynamic (and magnetohydrodynamic) story of knotted fields is well developed. We have given a sketch, but the reader is invited to find more detail in reviews such as Refs. \cite{Moffat, Irvine}. The point, however, is that the above discussion acts as a template for what one might expect in knotted fields more generally. Linking, self-linking, and their connection to conserved quantites in analogy to (\ref{eq:HelicityCount}) occur in a variety of contexts \cite{Winfree}; we will meet a result of the same structure in our study of twist-bend nematics in \S \ref{ch:3}. One particularly close cousin of the situation in fluids is that of superfluids, described by a complex scalar field $\psi = |\psi| e^{i \phi}$ (Figure \cite{fig:SuperFluidMontage}(a)) evolving via the non-linear Schroedinger equation \cite{}. Here vortices are given by singular lines where the circle-valued phase field $\phi$ is undefined, and about which it winds by $2\pi$. As in fluids, one may initialise these vortices in a knotted state and study their evolution (figure \cite{fig:SuperFluidMontage}(b)) \cite{Scheeler}, as well as defining some notion of helicity (although its precise form is more ambiguous \cite{Salman}). The helicity evolution turns out to be remarkably similar to that in real fluids \cite{Scheeler}. Coupled to this observation, it is found that superfluid knots also spontaneously untie over time, an example of which is shown in Figure \cite{fig:SuperFluidMontage}(b).
\begin{figure}[htbp]
\centering
...
\label{fig:RibbonMontage}
\end{figure}
\subsection{Liquid Crystals, Excitable Media, }
